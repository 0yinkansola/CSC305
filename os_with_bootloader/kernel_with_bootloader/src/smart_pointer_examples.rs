...
pub fn box_vs_rc() {
    ...
    //illustrate shared ownership in Rcs
    ...
    let y2 = y.clone(); // Creates a new reference to the same value. No memory duplication involved
                        ...
                        let _x1 = x.clone(); //Allocates a new Box in heap and deep copies the value. Memory duplication
                        let _x2 = x.clone(); //Allocates another new Box in heap and deep copies the value. Memory duplication
